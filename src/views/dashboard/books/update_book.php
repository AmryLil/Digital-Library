...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $query = "UPDATE books SET judul = ?, kategori = ?, deskripsi = ?, jumlah = ?, cover = ?, pdf = ? WHERE id = ?";
        $stmt = $pdo->prepare($query);
        $stmt->execute([$title, $category, $description, $quantity, $cover, $pdf, $bookId]);
                ...
                <form method="POST" enctype="multipart/form-data" class="space-y-4 w-full p-4">
                    ...
                    <div>
                        ...
                        <select id="category" name="category" required
                            ...
                            <option value="Fantasy" <?php echo $book['kategori'] === 'Fantasy' ? 'selected' : ''; ?>>Fantasy</option>
                            <option value="Romance" <?php echo $book['kategori'] === 'Romance' ? 'selected' : ''; ?>>Romance</option>
                            <option value="Mystery" <?php echo $book['kategori'] === 'Mystery' ? 'selected' : ''; ?>>Mystery</option>
                            <option value="Thriller" <?php echo $book['kategori'] === 'Thriller' ? 'selected' : ''; ?>>Thriller</option>
                            <option value="History" <?php echo $book['kategori'] === 'History' ? 'selected' : ''; ?>>History</option>
                            <option value="Science" <?php echo $book['kategori'] === 'Science' ? 'selected' : ''; ?>>Science</option>
                            <option value="Technology" <?php echo $book['kategori'] === 'Technology' ? 'selected' : ''; ?>>Technology</option>
                            <option value="Education" <?php echo $book['kategori'] === 'Education' ? 'selected' : ''; ?>>Education</option>
